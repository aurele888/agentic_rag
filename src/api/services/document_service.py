...
class DocumentService:
    ...
    async def upload_and_process(
        ...
        # Generate unique ID
        current_document_type = [file_type.name for file_type in SupportedFileType if file_type.value == Path(file_path).suffix]
        pdf_id = self._generate_pdf_id(file.filename, current_document_type[0])
        # Save file
    ...
    def _generate_pdf_id(self, filename: str, doc_type: str) -> str:
        ...
        return f"{doc_type}_{abs(hash(filename + timestamp))}"
